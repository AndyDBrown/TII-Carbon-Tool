...
useremisgreen_ui <- function(id,tabName){
  ns <- NS(id)
    ...
    box(width = 12,
        ...
        fluidRow(
          ...
          column(width = 5, align = "right",
                 ...
                 h6(HTML("<br>")),
                 h4(tagList(eflibrary_icon, paste0("Total ", roadorrail," User Emissions (tCO2):  "), textOutput(outputId = ns("sum_useremis_tCO2"), inline = T), 
                            HTML("&nbsp;"), HTML("&nbsp;")))
          )
        ),
                        ...
                        box(width = 12,
                            ...
                            hidden(
                              div(id = ns("guid_notes"),
                                  p(HTML(paste0("<br> </br>The <b> Greenways User Emissions</b> page calculates emissions avoided as a result of the new greenway.<br><br>
                          ...
                          The data input tables require:<br>
                            <ul><li>Input of use data direct from the TEAM Tool for project lifetime</li></ul><br>
        ...
        tabBox(width = 12, side = "left", id = ns("tabbox"), selected = "vehicleuse",
               ...
               tabPanel(title = tagList("Modal Shift", icon("arrow-circle-right")), value = "vehicleuse",
                        fluidRow(
                          column(width = 12,
                                 tags$div(title="This is change in CO2 as a result of the greenways CO2 savings",#"This is Annual Emissions multiplied by Scheme Design Life",
                                          h4("Change in Emissions Due to Modal Shift (tCO2):  ", textOutput(outputId = ns("sum_vehu_tCO2"), inline = T), tags$sup(icon("question-circle")))
                                 ),
                                 #h4(HTML("<br><br>")),
                                 ...
                                 # p(HTML("* Please enter a <b>positive</b> value avoided CO2."))
                                 column(6,offset = 6,
...
useremisgreenway_server <- function(id, option_number, thetitle, theoutput, appR_returned, projectdetails_values){
  

  
  DF_vehuse = data.table(`TEAM Tool Outputs: tonnes CO2 avoided` = c(0.0),
                         `Comments` = "",
  ...
  sum_vehu_tCO2 <- reactiveValues(data=0.0) # as col 1 - TEAM Tool Outputs: tonnes CO2 avoided
               ...
               function(input, output, session){
                 ...
                 observeEvent(appR_returned$loadReact, {
                   ...
                   if (is.null(appR_returned$data[[paste0(id, "_vehuTbl")]])# ||
                       # stringr::str_detect(id, pattern = "ro[0-9]{1,2}$") ||
                       # stringr::str_detect(id, pattern = "ra[0-9]{1,2}$")
                       ){
                   ...
                   sum_vehu_tCO2$data = sum(na.omit(tmpData$`TEAM Tool Outputs: tonnes CO2 avoided`)) * -1 # negative becayse it's tonne savoided
                 ...
                 observeEvent(input$vehu_mod_row_head, {
                   ...
                   showModal(modalDialog(
                     title = "Add a new row",
                     numericInput(ns("vehu_add_col1"), label = "TEAM Tool Outputs: tonnes CO2 avoided", value = vehuvalues$data[input$vehuTbl_rows_selected,1], min = 0),
                     textInput(ns("vehu_add_col2"), label = "Comments", value = vehuvalues$data[input$vehuTbl_rows_selected,2]),
                 ...
                 observeEvent(input$vehu_add_row_go, {
                   ...
                   sum_vehu_tCO2$data = sum(na.omit(vehuvalues$data$`TEAM Tool Outputs: tonnes CO2 avoided`))
                 ...
                 output$vehu_Template_download <- downloadHandler(
                   filename = function(){"Greenway_UserEmissions.xlsx"},
                   content =function(file){file.copy(from = "Templates/Greenway_UserEmissions.xlsx",to = file)}
                 )
                 ...
                 observeEvent(input$vehu_Template_upload$name, {
                   ...
                   if (identical(names(templateIn)[1:2], names(vehuvalues$data)[1:2])){
